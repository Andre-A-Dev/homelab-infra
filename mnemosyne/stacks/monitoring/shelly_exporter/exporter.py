...
Polls Shelly Plug S (Gen1 + Gen2/3) via local HTTP REST API.
...
TEMP    = Gauge("shelly_device_temp_celsius", "Internal device temperature (°C)", ["device", "host"], registry=registry)
...
def parse_devices() -> list[dict]:
    """
    Reads SHELLY_DEVICES env var.
    Format: "name:host:gen,name:host:gen"
    ...
    for entry in raw.split(","):
        ...
        parts = entry.split(":")
        if len(parts) != 3:
            log.warning("Skipping malformed device entry: %s", entry)
            continue
...
def poll_gen1(device: dict) -> None:
    ...
    try:
        ...
        if "tmp" in data:
            TEMP.labels(*labels).set(data["tmp"].get("tC", 0))
...
def poll_gen2(device: dict) -> None:
    """Poll Gen2/3 devices via RPC endpoint."""
    ...
    try:
        ...
        if "temperature" in data:
            TEMP.labels(*labels).set(data["temperature"].get("tC", 0))
...
def poll_loop(devices: list[dict]) -> None:
    while True:
...
if __name__ == "__main__":
    ...
    if not devices:
        log.error("No devices configured. Set SHELLY_DEVICES env var.")
    else:
        log.info("Starting polling for %d device(s): %s", len(devices), [d["name"] for d in devices])
        t = threading.Thread(target=poll_loop, args=(devices,), daemon=True)
        t.start()
